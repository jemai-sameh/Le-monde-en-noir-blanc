...
var pageConten = document.getElementById('pageContent');
var pictInf = document.getElementById('pictInfo');
...
let image = JSON.parse(rawImages);
let imageRandom = image[Math.floor(Math.random() * image.length)];

pageConten.style.backgroundImage = 'url(/web/css/images/' + imageRandom.filename + ')';
if (pictInf) {
    while (pictInf.firstChild) {
        pictInf.removeChild(pictInf.firstChild);
    }
    console.log(imageRandom);

    let pictTitle = document.createElement('h1');
    let pictTitleContent = document.createTextNode(imageRandom.name);
    pictTitle.appendChild(pictTitleContent);
    pictInf.appendChild(pictTitle);

    let pictAuthor = document.createElement('p');
    let pictAuthorContent = document.createTextNode(imageRandom.author);
    pictAuthor.appendChild(pictAuthorContent);
    pictInf.appendChild(pictAuthor);

    let pictExposure = document.createElement('p');
    pictInf.appendChild(pictExposure.appendChild(document.createTextNode(imageRandom.exposure)))
}